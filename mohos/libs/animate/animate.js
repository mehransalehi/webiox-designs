...
function initElements(){
  document.querySelectorAll('[asoop-animation]').forEach((e)=>{
    let animation = JSON.parse(e.getAttribute("asoop-animation"));
...
function animate(node,type){
  let animation = JSON.parse(node.getAttribute("asoop-animation"));
...
addEventListener('scroll', (event) => {
  document.querySelectorAll('[asoop-paralex-animation]').forEach((e)=>{
...
function translate(node,res){
  ...
  let animation = JSON.parse(node.getAttribute("asoop-paralex-animation"));
  let ruleStr = '';
